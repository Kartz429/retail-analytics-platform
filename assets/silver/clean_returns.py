...
Silver layer cleaning for the `returns` dataset.
...
def clean_returns(df: pd.DataFrame) -> pd.DataFrame:
    """Apply all Silver-layer cleaning rules to a raw Bronze returns DataFrame."""
    row_count_before = len(df)
    logger.info("Row count before cleaning: %d", row_count_before)

    # Remove null / duplicate return_id
    ...
    if "refund_amount" in df.columns:
        ...
        logger.info("  - dropped %d rows with refund_amount <= 0 or invalid", before - len(df))

    # Standardize text fields
    if "return_reason" in df.columns:
        df["return_reason"] = standardize_text(df["return_reason"])
    if "refund_status" in df.columns:
        df["refund_status"] = standardize_text(df["refund_status"])

    # Remove any remaining invalid/incomplete records
    required_cols = [c for c in ["return_id", "return_reason", "refund_status"] if c in df.columns]
    if required_cols:
...
def process_partition(partition_path: Path) -> None:
    ...
    logger.info("Processing returns partition: %s", partition_date)

    df = pd.read_parquet(partition_path)
    cleaned_df = clean_returns(df)
...
def main() -> None:
    ...
    if not partitions:
        logger.info("No new returns partitions to process.")
        return

    logger.info("Found %d new returns partition(s) to process.", len(partitions))
